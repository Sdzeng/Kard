...
namespace Kard.Dapper.Mysql.Repositories
{
    ...
    public class DefaultRepository : IDefaultRepository
    {
        ...
        public DefaultRepository(IConfiguration configuration, ILogger<DefaultRepository> logger)
        {
        ...
        {
            return Task.FromResult(Delete<T>(entity, commandTimeout));
        }
        ...
        public Task<ResultDto> DeleteAsync<T>(object predicate, int? commandTimeout = null, bool isPhysics = false) where T : class
        {
            return Task.FromResult(Delete<T>(predicate, commandTimeout, isPhysics));
        }
